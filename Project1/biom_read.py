...
def break_table(filepath, betas, lean, obese):
    ...
    # Save the Bray-Curtis-Faith distances to output csvs,
    with open(filepath+"/stats/LL_stats.csv", "wb") as f:
        wr = csv.writer(f,quoting=csv.QUOTE_ALL)
        wr.writerow(LL_stats)
        f.close()
    with open(filepath+"/stats/OO_stats.csv", "wb") as f:
        wr = csv.writer(f,quoting=csv.QUOTE_ALL)
        wr.writerow(OO_stats)
        f.close()
    with open(filepath+"/stats/LO_stats.csv", "wb") as f:
        wr = csv.writer(f,quoting=csv.QUOTE_ALL)
...
def boxplots(filepath, LL_stats, OO_stats, LO_stats):
    ...
    fig.savefig(filepath+"/figures/boxplot.png", bbox_inches='tight')
...
def distribution(filepath,LL_stats,OO_stats,LO_stats):
    ...
    pl.title("Bray-Curtis-Faith Dissimilarity of Lean vs Lean Twins")
    pl.savefig(filepath+"/figures/LL_Distribution.png")
    pl.clf()

    # Plot the Obese vs Obese
    pl.plot(OO,OO_fit,'-o')
    pl.hist(OO,normed=True)
    pl.title("Bray-Curtis-Faith Dissimilarity of Obese vs Obese Twins")
    pl.savefig(filepath+"/figures/OO_Distribution.png")
    pl.clf()

    #Plot the Lean vs Obese
    pl.plot(LO,LO_fit,'-o')
    pl.hist(LO,normed=True)
    pl.title("Bray-Curtis-Faith Dissimilarity of Lean vs Obese Twins")
    pl.savefig(filepath+"/figures/LO_Distribution.png")
    pl.clf()
